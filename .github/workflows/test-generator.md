...
### 7b — Commit Generated Files

Replace `<feature-basename>` and `<feature-file-path>` with the actual values derived in Step 7a and Step 0.

```bash
git config user.name  "github-actions[bot]"
git config user.email "github-actions[bot]@users.noreply.github.com"
git checkout -b test-gen/<feature-basename>
git add steps/pages/
git commit -m "test(gen): generate step definitions for <feature-basename>

Auto-generated by the test-generator agentic workflow.
Source: <feature-file-path>"
git push origin test-gen/<feature-basename>
```

### 7c — Open Pull Request via GitHub MCP

Use the GitHub MCP `create_pull_request` tool with:

- **title**: `test(gen): step definitions for <feature-basename>`
- **head**: `test-gen/<feature-basename>`
- **base**: `main`
- **body**:
...
- **Do not push directly to `main`.** Always use a `test-gen/<name>` branch and open a PR.
